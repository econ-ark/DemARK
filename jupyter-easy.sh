...
docker exec $CONTAINER_ID pkill -f jupyter-lab 2>/dev/null || true
sleep 2

# Start Jupyter Lab in the existing container
...
if docker exec $CONTAINER_ID pgrep -f jupyter-lab > /dev/null 2>&1; then
    ...
    echo ""
    echo "  docker port-forward $CONTAINER_ID $PORT:$PORT"
    echo ""
    echo "Or use this simpler command:"
    echo ""
    echo "  docker exec -it $CONTAINER_ID socat TCP-LISTEN:$PORT,fork TCP:localhost:$PORT"
    echo ""
    ...
    echo "🎉 Ready to use!"
    echo ""
    echo "🛑 To stop Jupyter Lab: docker exec $CONTAINER_ID pkill -f jupyter-lab"
else
